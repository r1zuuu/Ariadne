...
// The names in a question used to be read out of it twice: by a regex over the
// query, and by this model, which was the only one of the two that could know
// Hono is a library. The second reading is gone. It found the names no shape
// gives away, and it cost about half a second on every search that ever ran,
// answered or not - the numbers are in eval/results.md, and the trade is
// written up in docs/rag-case-study.md. literalsByShape in service.ts is now
// the whole of it.
